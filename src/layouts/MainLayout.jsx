...
import {
  selectUser, selectIsAuthenticated, toggleDarkMode, selectDarkMode,
  selectActiveAddress, selectShowLocationModal, setActiveAddress, setShowLocationModal
...
const MainLayout = ({ children, customToast }) => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          dispatch(setActiveAddress(friendlyAddress));
          dispatch(setShowLocationModal(false));
          ...
          dispatch(setActiveAddress(fallbackAddr));
          dispatch(setShowLocationModal(false));
  ...
  return (
    <div className={`min-h-screen flex flex-col ${darkMode ? 'dark text-slate-100 bg-slate-950' : 'bg-slate-50 text-slate-800'}`}>
      ...
      {/* Top Navbar */}
      <nav className={`z-40 transition-all duration-500 ${isLandingPage
          ? !isScrolled
            ? 'absolute top-0 left-0 right-0 bg-transparent border-transparent text-white'
            : 'fixed top-0 left-0 right-0 bg-white/80 dark:bg-slate-950/80 backdrop-blur-lg border-b border-slate-100/50 dark:border-slate-900/60 shadow-sm text-slate-800 dark:text-slate-200'
          : 'sticky top-0 bg-white/80 dark:bg-slate-950/80 backdrop-blur-lg border-b border-slate-100/30 dark:border-slate-900/40 shadow-sm text-slate-800 dark:text-slate-200'
        }`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 sm:h-20 gap-4">
            ...
            <Link to="/" className="flex items-center space-x-2 flex-shrink-0 group" onClick={() => dispatch(updateFilter({ key: 'search', value: '' }))}>
              ...
              <span className={`text-xl sm:text-2xl font-black font-display tracking-tight transition-colors duration-500 ${isLandingPage && !isScrolled ? 'text-white' : 'text-slate-900 dark:text-white'
                }`}>
            ...
            <div className="flex items-center space-x-1 sm:space-x-3">
              ...
              <button
                ...
                className={`p-2.5 rounded-2xl transition-all border border-transparent ${isLandingPage && !isScrolled
                    ? 'text-white hover:bg-white/10'
                    : 'text-slate-500 dark:text-slate-455 transition-colors border border-transparent hover:border-slate-200/20'
                  }`}
              ...
              <Link
                ...
                className={`p-2.5 rounded-2xl transition-all relative border border-transparent ${isLandingPage && !isScrolled
                    ? 'text-white hover:bg-white/10'
                    : 'text-slate-500 dark:text-slate-455 transition-colors relative border border-transparent hover:border-slate-200/20'
                  }`}
              >
              ...
              {isAuthenticated ? (
                <Link
                  to="/dashboard"
                  className={`flex items-center justify-center p-2.5 rounded-2xl transition-all border border-transparent ${isLandingPage && !isScrolled
                      ? 'text-white hover:bg-white/10'
                      : 'text-slate-500 dark:text-slate-450 hover:text-emerald-650 dark:hover:text-emerald-400 hover:bg-slate-100 dark:hover:bg-slate-900'
                    }`}
                >
                ...
                <Link
                  to="/login"
                  className={`hidden md:inline-flex items-center justify-center px-4 py-2.5 rounded-2xl transition-all text-xs font-black border ${isLandingPage && !isScrolled
                      ? 'bg-white/15 text-white hover:bg-white/25 border-white/20'
                      : 'bg-slate-100 dark:bg-slate-900 text-slate-700 dark:text-slate-200 hover:bg-emerald-50 dark:hover:bg-emerald-950/20 hover:text-emerald-650 dark:hover:text-emerald-400 border-transparent hover:border-emerald-500/20'
                    }`}
                >
                  Sign In
                </Link>
      ...
      <main className={`flex-grow w-full ${isLandingPage ? '' : 'max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8'} mb-16 md:mb-0`}>
        {children}
      </main>
      ...
      {showLocationModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 w-full max-w-md border border-slate-100 dark:border-slate-805 shadow-2xl relative">
            <button
              onClick={() => dispatch(setShowLocationModal(false))}
            ...
            <div className="space-y-2 mb-4">
              ...
              */}
              ...
              <button
                ...
                className="w-full mb-3 p-3.5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 disabled:from-blue-400 disabled:to-indigo-400 text-white rounded-2xl flex items-center justify-center space-x-2 text-xs font-black transition-all shadow-md shadow-blue-500/20 active:scale-95 cursor-pointer"
              ...
              {locations.map((loc, idx) => (
                <button
                  ...
                  className={`w-full p-3.5 text-left rounded-2xl flex items-center space-x-3 border text-xs font-bold transition-all ${activeAddress === loc
